...
import { LinkButton, SecondaryButton } from '../../Components/styledComponents/Buttons';

export default function AdminNav() {

    const lists=["Dashboard","Trainers","Students","Logout"]
    ...
    const linksLists=lists.map((i,ind)=>{
        ...
        return(
            <Link to={`/admindashboard/${i.toLocaleLowerCase()}`} key={ind}>
  ...
  const list = (anchor) => (
    ...
    >
      ...
      >
        {linksLists}
        <SecondaryButton onClick={()=>{localStorage.removeItem('token');navigate(`/`,{replace:true})}} sx={{width:"100%"}}>{"Logout"}</SecondaryButton>                     
        </Stack>
