...
const rows = 
  studentList.map((i)=>(createData(`${i.name}`,`${i.email}`,`${i.isVerfied?'Verified':'Not Verified'}`,`${i._id}`)))
...
console.log(studentList)
  ...
  async function handleDelete(params) {
    try {
        await axios.delete(`http://localhost:8000/admin/deleteStudent/${params}`,header)
...
async function name(params) {
    try {
      const res = await axios.get(`http://localhost:8000/admin/students`,header)
...
React.useEffect(() => {
  name()
    ...
    <Paper sx={{ width: '80vw', overflow: 'hidden',margin:'auto',mt:2 }}>
                    ...
                    {columns.map((column) => {
                      const value = row[column.id];
                        if(column.id=="action"){
                          ...
                          <SecondaryButton onClick={()=>handleEdit(value)}>Edit</SecondaryButton>
                          <SecondaryButton onClick={()=>handleDelete(value)}>Delete</SecondaryButton>
                        </TableCell>
                            )
                        }
                      return (
      ...
      <Button onClick={()=>{navigate(`/admindashboard/editstduents/add`)}}>Add Student</Button>
    </Box>
  )
}
