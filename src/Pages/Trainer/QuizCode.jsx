...
export default function QuizCode({ changeEdit }) {
  ...
  async function putQuiz(params) {
    ...
    try {
      let res = await axios.put(
        `https://quiz-backend-cw2w.onrender.com/trainer/putQuiz/${params}`,
        obj,
        header,
      );
      if (res.status == 200) {
  ...
  async function handleCreate(params) {
    ...
    try {
      let res = await axios.post(
        "https://quiz-backend-cw2w.onrender.com/trainer/quiz",
        obj,
        header,
      );
      if (res.status == 200) {
  ...
  async function deleteQuiz(params) {
    try {
      await axios.delete(
        `https://quiz-backend-cw2w.onrender.com/trainer/quiz/${params}`,
        header,
      );
  ...
  async function getQuiz(params) {
    try {
      ...
      if (res.status == 200) {
        dispatch(getQuizzes(res.data.quiz));
      }
    } catch (error) {
      enqueueSnackbar("Server Error", { variant: "error" });
      console.log(error.message);
  ...
  async function deploy(params) {
    try {
      let res = await axios.put(
        `https://quiz-backend-cw2w.onrender.com/trainer/quiz/${params.id}`,
        { isPublished: !params.isPublished },
        header,
      );
      enqueueSnackbar(
        `Quiz ${params.isPublished ? "UnPublished" : "Published"}`,
        { variant: "success" },
      );
  ...
  return (
    <div>
      ...
      <CustomTable
        ...
        publish={(rows) => {
          return (
            <SecondaryButton onClick={() => deploy(rows)}>
              {rows.isPublished ? "UnPublish" : "Publish"}
            </SecondaryButton>
          );
        }}
        open={(rows) => {
        ...
        view={(rows) => {
          return (
            <SecondaryButton
              onClick={() =>
                navigate(`/trainerdashboard/quiz/result/${rows.id}`)
              }
            >
              View
            </SecondaryButton>
